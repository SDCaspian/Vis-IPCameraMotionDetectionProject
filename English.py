...
class MotionDetector:
    def __init__(self, url, camera_name="Please enter the name of camera", threshold=5000, min_area=500, save_dir="motion_captures"):
        ...
        if not os.path.exists(save_dir):
            ...
            print(f"[{self.get_time()}] Created photo save directory: {save_dir}")
    ...
    def keep_alive_heartbeat(self):
        print(f"[{self.get_time()}] Heartbeat thread started (interval {self.heartbeat_interval}s)")
        endpoints = [
            self.url,
        ...
        while self.keep_alive_running:
            try:
                ...
                if self.heartbeat_count % 10 == 0:
                    print(f"[{self.get_time()}] Heartbeat #{self.heartbeat_count} - Device online")
            except requests.exceptions.Timeout:
                print(f"[{self.get_time()}] Heartbeat timeout, device may be in sleep mode")
            except requests.exceptions.ConnectionError:
                print(f"[{self.get_time()}] Heartbeat connection failed, device may be offline")
            except Exception as e:
                if self.heartbeat_count % 10 == 0:
                    print(f"[{self.get_time()}] Heartbeat error: {e}")
            time.sleep(self.heartbeat_interval)
        print(f"[{self.get_time()}] Heartbeat thread stopped")
    
    def connect(self):
        print(f"[{self.get_time()}] Connecting to camera...")
        try:
            ...
            if wake_response.status_code == 200:
                print(f"[{self.get_time()}] Wake-up request sent")
            wake_response.close()
        except:
            pass
        ...
        if not self.cap.isOpened():
            ...
            print(f"[{self.get_time()}] Connection failed ({self.reconnect_attempts}/{self.max_reconnect_attempts})")
        ...
        self.reconnect_attempts = 0
        self.last_frame_time = time.time()
        print(f"[{self.get_time()}] Connection successful!")
        return True
    ...
    def check_and_reconnect(self):
        ...
        if now - self.last_frame_time > self.frame_timeout:
            print(f"[{self.get_time()}] Connection timeout detected ({now - self.last_frame_time:.1f}s no new frame)")
            print(f"[{self.get_time()}] Attempting to reconnect...")
            if self.cap:
                self.cap.release()
            return self.connect()
        return True
    ...
    def detect_motion(self, frame):
        ...
        if motion_detected:
            ...
            if not self.is_moving:
                ...
                print(f"   Areas: {len(motion_areas)}")
                print(f"   Max area: {max_area:.0f} pixels")
    ...
    def print_statistics(self):
        ...
        print(f"Statistics")
        print(f"{'='*60}")
        print(f"   Motion events detected: {self.total_motion_events}")
        print(f"   Auto saved photos: {self.auto_save_count}")
        print(f"   Manual saved photos: {self.manual_save_count}")
        print(f"   Heartbeat requests: {self.heartbeat_count}")
        print(f"   Save directory: {os.path.abspath(self.save_dir)}")
        print(f"{'='*60}")
    
    def run(self):
        if not self.connect():
            print("Unable to connect to camera, please check network and address")
            return False
        heartbeat_thread = threading.Thread(target=self.keep_alive_heartbeat, daemon=True)
        heartbeat_thread.start()
        print(f"\n[{self.get_time()}] Monitoring system running...")
        print("Controls:")
        print("  'q' - Quit program")
        print("  's' - Save current frame")
        print("  'h' - Manual heartbeat trigger")
        ...
        while True:
            ...
            if not ret:
                ...
                print(f"[{self.get_time()}] Frame read failed ({consecutive_failures}/3)")
                if consecutive_failures >= 3:
                    print(f"[{self.get_time()}] Consecutive failures, triggering reconnect...")
                    self.connect()
                    consecutive_failures = 0
                continue
            consecutive_failures = 0
            try:
                ...
                print(f"[{self.get_time()}] Motion detection error: {e}")
                display_frame = frame
            cv2.imshow('IP Camera', display_frame)
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                print(f"\n[{self.get_time()}] User exited monitoring system")
                break
            elif key == ord('s'):
                ...
                print(f"[{self.get_time()}] Manual saved: {filename}")
            elif key == ord('h'):
                print(f"[{self.get_time()}] Manual heartbeat trigger...")
                try:
                    requests.get(self.url, timeout=3, 
                               auth=requests.auth.HTTPBasicAuth('admin', '12345678'))
                    print(f"[{self.get_time()}] Heartbeat sent successfully")
                except Exception as e:
                    print(f"[{self.get_time()}] Heartbeat failed: {e}")
...
def main():
    print("="*60)
    print("IP Camera - Monitoring System")
    print("="*60)
    
    ip_address = input("Please enter camera IP address (default: 192.168.1.1): ").strip()
    if not ip_address:
        ip_address = "192.168.1.1"
    
    camera_name = input("Please enter camera name (default: Please enter the name of camera): ").strip()
    if not camera_name:
    ...
    try:
        ...
        print(f"\n[{detector.get_time()}] Program interrupted by user")
        detector.print_statistics()
    except Exception as e:
        print(f"[{detector.get_time()}] Error occurred: {e}")
        import traceback
        traceback.print_exc()
